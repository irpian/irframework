...
if(is_dir(fixPath($path))){
  if(!empty($_FILES['files']) && is_array($_FILES['files']['tmp_name'])){
    ...
    foreach($_FILES['files']['tmp_name'] as $k=>$v){
      ...
      // elseif(!$_FILES['files']['size'][$k] > 650000){
      //   $errorsSze[] = $filename;
      //   $isUploaded = false;
      //   die("error ukuran");
      // } //ukuran file
    ...
    // elseif($errorsSze) //irpie
    //   $res = getSuccessRes(t('E_UploadEsize')); //irpie
    else
      $res = getSuccessRes();
  }
  else
...
if($isAjax){
  if($errors || $errorsExt) //if($errors || $errorsExt || $errorsSze) //irpie 
