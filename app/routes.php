...
class Route {
    ...
    function route($route="", $path_to_include=""){
      ...
      if(is_string($path_to_include) && preg_match('/.php/', $path_to_include)){
          $path_to_include = $this->theme->template($path_to_include);
      }

      $plus_request_path = 0;
      if($state!="frontend" && $route!="/"){ 
        $route = "/".$state."/".$modules.$route;
        $plus_request_path = 1;
      } elseif($route!="/"){
        $route = "/".$modules.$route;
        $plus_request_path = 2;
      ...
      if($state!="frontend") { 
        if($modules!=$config['module']['default']){
          $request_parts_number = 2 + $plus_request_path;  
        } else {
