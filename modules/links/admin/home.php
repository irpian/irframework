            ...
            <div class="panel-body">
                <table class="table">
                    ...
                    <?php foreach($list_account as $data) { 
                        $share = $db->row("id", "links", "WHERE account='".$data['id']."' AND status=1");
                        $pending = $db->row("id", "links", "WHERE account='".$data['id']."' AND status=0");
                        $total = $db->row("id", "links", "WHERE account='".$data['id']."' ");
                    ?>
                    <tr>
